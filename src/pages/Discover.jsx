...
const Discover = () => {
    ...
    const { data, isFetching, error } = useGetSongsByGenreQuery(genreListId || 'POP') //data: resource cua Api call, isFetching propoties: show the loading state trạng thái chờ ?, và error
    console.log(data)
    if(isFetching) return <Loader title="Loading songs..." />
    
    if(error) return <Error/>
    
    const genreTitle = genres.find(({value}) => value === genreListId)?.title //title of Discover
    return (
