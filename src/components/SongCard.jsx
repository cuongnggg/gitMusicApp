...
const SongCard = ({ song, isPlaying, activeSong ,i, data }) => {
  ...
  //w-[250px] dẫn tham số thẳng vào width
  return (
    <div className='flex flex-col w-[250px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slide-up rounded-lg cursor-pointer'>
      <div className='relative w-full h-56 group'>
        ...
        ${activeSong?.title === song.title ? 'flex bg-black bg-opacity-70' : 'hidden'}`}>
          {/* khi hover vào có thể play nhạc hoặc dừng */}
          <PlayPause
            isPlaying = {isPlaying}
      ...
      <div className='mt-4 flex flex-col'>
        <p className='font-semibold text-lg text-white truncate'> {/*truncate: khi text quá dài sẽ được ẩn or ... */}
